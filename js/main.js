...
function createStatusDisplay() {
  ...
  for (let i = 0; i < textLabels.length; i++) {
    ...
    textGroup.renderOrder = 53;
    
    const textMesh = createTextureText(textLabels[i], 0.15);
    textMesh.renderOrder = 53;
    textGroup.add(textMesh);
    
    textGroup.userData = { id: textIds[i] };
    textGroups.push(textGroup);
    displayGroup.add(textGroup);
  }
  
  // ステータス表示オブジェクトのユーザーデータにテキストグループを保存
  displayGroup.userData = {
    textGroups: textGroups,
    ring: ring, // 回転アニメーション用に保存
    rotationSpeed: 0.001 // 回転速度
  };
...
function init() {
    ...
    mainLight.shadow.mapSize.width = 512;  // シャドウマップのサイズを最適化
